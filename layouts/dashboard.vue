...
export default mixins(login).extend({
  ...
  watch: {
    $route() {
      ...
      this.$store.commit('SET_PAGE_NAME', { name: this.$route.path.substring(1) })
    },
  },
  mounted() {
  ...
  async created(){
    let redirectStore = localStorage.getItem("redirect_paperdaz_path")
     await this.filterUsers()
    if(localStorage.getItem("redirect_paperdaz_path")){
     localStorage.removeItem("redirect_paperdaz_path")
     this.$nuxt.$router.push(redirectStore)
    }
  },
  beforeCreate(){
    // console.log("dalete jsmvksvm,")
  }
})
</script>
