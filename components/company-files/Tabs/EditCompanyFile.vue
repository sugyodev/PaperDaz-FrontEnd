<template>
  <el-dialog :visible.sync="showModal" :append-to-body="true" style="" :show-close="false" center
    ...
    <!--End:: Close Button -->
    <template #title>
      <h4 class="text-center font-semibold text-xl">Edit File Name</h4>
    </template>
    <!-- Start:: Body -->
    ...
    <div class="flex justify-around">
      <button class="disabled:bg-opacity-50 disabled:cursor-progress h-10 text-xs w-[900%] 
        shadow-md
         text-white rounded-lg bg-paperdazgreen-400" :disabled="loading" @click="onSubmit">
...
export default Vue.extend({
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.fileInputData.trim().length < 1) return
