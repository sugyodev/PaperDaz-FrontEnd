  ...
  <el-dialog :visible.sync="showModal" :append-to-body="true" style="" :show-close="false" center
    ...
    <form @submit.prevent="onSubmit" action="" class="max-w-[470px] mx-auto">
      <div class="grid grid-cols-[1fr,min-content] items-center gap-3 mb-8">
        <div>
          <input v-for="mail in emails" :key="email" placeholder="Email" type="email" required
            class="my-1 p-3 w-full border-[1px] border-grey rounded-[7px] outline-green resize-none" />
        </div>

        <div @click="addEmails" class="circle circle-18 bg-gray-600 text-white font-medium text-xl cursor-pointer">
          +
        </div>
      </div>

      <el-input type="textarea" :rows="6" placeholder="Description" v-model="description" class="resize-none">
      </el-input>
      <div class="flex justify-around pt-5">
        <button class="disabled:bg-opacity-50 disabled:cursor-progress h-10 text-xs w-[100%] 
         text-white rounded-md shadow bg-paperdazgreen-400" :disabled="loading">
